...
export default function AdminProductsPage() {
  ...
  const STOREFRONT_COLLECTIONS = useMemo(
    () => [
  ...
  function openEditModal(p: Product) {
    ...
    setForm({
      ...
      collection: p.collection ?? 'clothing',
  ...
  async function onSave(e: React.FormEvent) {
    ...
    try {
      ...
      const payload = {
        ...
        is_partywear: form.collection === 'partywear' ? true : form.is_partywear,
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    <span
                      className={cn(
                        ...
                        getCollectionBadgeClass(p.collection)
                      )}
                    >
                      {getCollectionLabel(p.collection)}
                    </span>
                    ...
                    {p.is_bestseller ? (
                      <span className="rounded-full bg-yellow-50 px-2 py-0.5 text-[9px] font-bold text-yellow-700">
                        Favorite
                      </span>
                    ) : null}

                    {p.gender?.map((g) => (
                      <span key={g} className="rounded-full bg-blue-50 px-2 py-0.5 text-[9px] font-bold text-blue-600">
                        {g}
                      </span>
                    ))}
                  </div>
      ...
      <Modal open={modalOpen} title={modalMode === 'add' ? 'New Creation' : 'Update Details'} onClose={closeModal}>
        <form onSubmit={onSave} className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  const nextCollection = e.target.value as StorefrontCollection;

                  setForm((prev) => ({
                    ...prev,
                    collection: nextCollection,
                    is_partywear: nextCollection === 'partywear' ? true : false,
                  }));
                }}
                className="w-full rounded-xl border border-[#ead8cd] p-3 text-sm"
              >
          ...
          <div>
            ...
            <div className="mt-2 space-y-4 rounded-2xl border border-[#ead8cd] bg-[#fdf7f2] p-4">
              <div>
                <div className="text-[11px] font-bold text-[#4b3b33]">Quick pick (months)</div>

                <div className="mt-2 flex flex-wrap gap-2">
                  {[3, 6, 9, 12, 18, 24, 36, 48, 60, 72, 84, 96, 108, 120, 132, 144, 156, 168].map((m) => {
                      ...
                      <button
                        ...
                        onClick={() =>
                          setForm((prev) => ({
                            ...
                            age_months: normalizeAgeMonths(
                              prev.age_months.includes(m)
                                ? prev.age_months.filter((x) => x !== m)
                                : [...prev.age_months, m]
                            ),
                          }))
                        }
                        className={cn(
                          ...
                          active
                            ...
                            : 'border-[#ead8cd] bg-white text-[#4b3b33] hover:bg-[#fff7f0]'
                        )}
                      >
              ...
              <div className="border-t border-[#ead8cd] pt-4">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  <div>
                    ...
                    <p className="text-[11px] text-gray-600">
                      Example: <span className="font-mono">12,18</span>
                    </p>
                    <input
                      type="text"
                      inputMode="numeric"
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={() => {
                      const computed = computeAgeMonthsFromTexts(
                        form.age_month_points_text,
                        form.age_year_points_text
                      );
                ...
                <div className="mt-3 rounded-xl border border-[#ead8cd] bg-white p-3">
                  ...
                  <div className="mt-2 break-words font-mono text-xs text-gray-700">
          ...
          <div>
            ...
            <div className="mt-2 space-y-3">
              ...
              <p className="text-[11px] text-gray-500">
                If you choose a file, it uploads via the admin API.
              </p>
            </div>
          </div>
            ...
            <div className="mt-3 space-y-3">
              <input
                ...
                onChange={(e) => {
                  ...
                  setForm((prev) => ({
                    ...prev,
                    gallery_files: [...prev.gallery_files, ...incoming],
                  }));
                }}
                className="w-full text-xs"
              />
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              onChange={(e) =>
                setForm((prev) => ({
                  ...prev,
                  is_partywear: e.target.checked,
                  collection: e.target.checked ? 'partywear' : prev.collection === 'partywear' ? 'clothing' : prev.collection,
                }))
              }
              className="h-5 w-5 rounded-md text-[#4b3b33]"
            />
            <label className="text-sm font-bold text-[#4b3b33]">Is this Partywear?</label>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <label htmlFor="bestseller" className="cursor-pointer text-sm font-bold text-[#4b3b33]">
